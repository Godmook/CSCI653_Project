...
def sched_pats(queue: List[Request], current_lora, current_time):
    """
    PATS (Perception-Aware Tetris Scheduler) - Corrected Implementation
    
    [Improvements]
    1. Dynamic Context Awareness: 
       배치 구성 중 첫 번째 요청이 결정되면, 그 요청의 LoRA를 'Simulated Current LoRA'로 간주합니다.
       따라서 배치 내의 후속 요청들은 스위칭 비용을 '0'으로 평가받아 같은 LoRA끼리 강력하게 뭉치게 됩니다.
    2. Sunk Cost Fallacy Fix:
       배치의 첫 리더가 스위칭 비용을 지불했다면, 팔로워들은 공짜로 탑승해야 합니다.
    """
    if not queue:
        return []
    
    # --- Hyperparameters ---
    # Alignment보다 Efficiency(같은 LoRA끼리 묶기 + Pruning)에 더 집중
    BETA = 0.5    
    GAMMA = 0.05  
    ...
    enable_pruning_mode = True 

    remaining = list(queue)
    batch = []

    # [핵심 수정] 배치를 채우는 동안 변화하는 GPU 상태를 추적
    # 초기에는 실제 GPU 상태, 배치가 차기 시작하면 배치 리더의 LoRA 상태
    simulated_lora_context = current_lora
    is_context_switched_in_this_batch = False

    # --- Iterative Greedy Packing ---
    while len(batch) < CLIFF_THRESHOLD and remaining:
        # Normalize Resource Vector (A)
        A = get_normalized_vector(residual_vram, residual_compute)
        
        # 리소스가 거의 꽉 찼으면 중단 (Floating point 이슈 방지)
        if A[0] <= 1e-6 or A[1] <= 1e-6: 
            break
        ...
        for i, req in enumerate(remaining):
            ...
            # Backfilling Check (Fit 여부 확인)
            if (residual_vram - d_vec[0] < 0) or (residual_compute - d_vec[1] < 0):
                continue 

            # 2. Efficiency & Dynamic Switching Cost
            # [핵심 로직 변경] 현재 배치의 문맥(simulated_lora_context)과 비교
            is_cached = (req.lora_id == simulated_lora_context)
            
            # 스위칭 비용 계산
            # 만약 이미 배치 리더가 스위칭을 결정했다면(is_context_switched_in_this_batch),
            # 그리고 이 요청이 리더와 같은 LoRA라면(is_cached), 추가 비용은 0이다.
            if is_cached:
                t_switch = 0.0
            else:
                # 배치가 비어있을 때만 스위칭 비용을 고려 (첫 타자)
                # 이미 배치가 찼는데 다른 LoRA가 들어오려 하면 페널티를 아주 강하게 줌
                if len(batch) == 0:
                    t_switch = req.switch_cost
                else:
                    # 이미 다른 LoRA로 배치가 시작되었는데, 또 다른 LoRA가 들어오려면
                    # 그건 이 배치가 끝나고 '다음 배치'에서나 가능한 일임 (혹은 엄청난 오버헤드)
                    # 따라서 매우 큰 페널티 부여
                    t_switch = req.switch_cost * 10.0 
            
            steps = PRUNED_STEPS if should_prune else FULL_STEPS
            t_compute = steps * STEP_TIME
            
            eff_time = t_switch + t_compute
            if eff_time <= 0: eff_time = 1e-3
            
            efficiency = 1.0 / eff_time

            # 3. Alignment (Cosine Similarity)
        ...
        # [Update Context]
        # 배치의 첫 번째 아이템이 들어갔다면, 이제부터 이 배치의 주인은 이 LoRA임.
        if len(batch) == 1:
            if best_req.lora_id != current_lora:
                is_context_switched_in_this_batch = True
            simulated_lora_context = best_req.lora_id
...
if __name__ == "__main__":
    ...
    output_path = "../data/scheduler_results_ablation2.csv"
    df.to_csv(output_path, index=False)
    print(f"\n📁 CSV saved to: {output_path}")

    timestamped_path = f"../data/scheduler_results_{int(time.time())}_ablation2.csv"
